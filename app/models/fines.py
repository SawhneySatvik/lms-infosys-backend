...
class Fine(db.Model):
    ...
    fine_id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    borrow_id = db.Column(UUID(as_uuid=True), db.ForeignKey('borrow_transactions.borrow_id', ondelete='CASCADE'), nullable=False)
    ...
    fine_date = db.Column(db.DateTime, default=datetime.now)
